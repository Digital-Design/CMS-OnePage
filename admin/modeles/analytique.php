...
function getGraphAnalytique(){
  $bdd = connectDB();
  $sql = "SELECT date_creation, COUNT(*) AS value, DATE_FORMAT(date_creation,'%Y-%m-%d') AS date_analytique FROM analytique WHERE date_creation >= DATE_SUB(CURDATE(), INTERVAL DAYOFWEEK(CURDATE())-1 DAY) GROUP BY date_analytique";
  $res = $bdd->prepare($sql);
  $res->execute();
  return $res->fetchAll(PDO::FETCH_ASSOC);
}
